...
//  - 跳过 IS Grid Search，直接用指定参数跑 OOS
//  - IS 期只计算固定参数的参考绩效，仅用于展示
//  - 目的：验证用户选定参数在从未见过的数据上的稳定性
//
// 【模式B：自动寻优 WFO（fixedParams = null 时，原有行为）】
//  - IS 跑 Grid Search（448种组合）→ 按 optMetric 选最佳参数
//  - 用该参数固定跑 OOS → 记录绩效
//  - 目的：无前视偏差的参数自动选择
//
// 窗口设计：单窗口 70% IS / 30% OOS
function runWFO(histData, commonTs, qqqCloses, optMetric='sharpe', fixedParams=null) {
  ...
  let bestParams;
  let inSampleScore    = null;
  let inSampleSharpe   = null;
  let inSampleCAGR     = null;
  let inSampleMDD      = null;
  ...
  if (isFixedMode) {
    ...
    // 计算固定参数在 IS 期的参考绩效（仅展示，不影响参数选择）
    try {
      const isBt = portfolioBacktest(histData, commonTs, qqqCloses, fixedParams, 0, inEnd);
      const isM  = calcPortMetrics(isBt.equityCurve, isBt.timestamps);
      if (isM) {
        inSampleSharpe = isM.sharpe;
        inSampleCAGR   = isM.cagr;
        inSampleMDD    = isM.mdd;
      }
    } catch(e) { /* IS 参考计算失败不影响 OOS */ }

  } else {
    // ── 模式B：IS 期 Grid Search（448种组合）──
...
function QqqSignalCard({ histData, histTs, params, T, darkMode }) {
    ...
    if (maDays > 0 && d >= maDays && qqqCloses) {
      ...
      if (ma != null && qqqNow != null && qqqNow <= ma) {
        return { date, isDefensive: true, holdings: {}, bufferHoldings: {}, prices: {},
          ...
          scores: [], rebalFreq: rebalanceFreq, qqqPrice: qqqNow, maDays, bufferEnabled, bufferN };
      }
    }
  ...
  if (signal && prevSignal && !isFirstSignal) {
    const prevAllSyms = new Set([
      ...Object.keys(prevSignal.holdings || {}),
      ...Object.keys(prevSignal.bufferHoldings || {}),
    ]);
    ...
    if (!wasDefensive && isDefensive) {
      for (const sym of prevAllSyms) {
        sellList.push({ sym, reason: '市场过滤触发 → 全仓转现金', price: prevSignal.prices?.[sym] });
      }
    }
    ...
    if (!wasDefensive && !isDefensive) {
      for (const sym of prevAllSyms) {
        if (currCoreSyms.has(sym)) {
          ...
          holdList.push({ sym, reason: `仍在Top${signal.topN}(#${rank})`, price: signal.prices?.[sym], color: '#4488ee' });
          ...
          const se = signal.scores?.find(s => s.sym === sym);
          const reason = se
            ? (se.score <= 0 ? '动能转负' : `跌出缓冲区(#${signal.scores.indexOf(se)+1} > Top${signal.bufferN})`)
            : '数据缺失';
      ...
      for (const sym of currAllSyms) {
        if (!prevAllSyms.has(sym)) {
          ...
          if (isCore) {
            buyList.push({ sym, reason: `新进Top${signal.topN}(#${rank}，+${se ? (se.score*100).toFixed(1) : '?'}%)`, price: signal.prices?.[sym] });
          } else {
            bufferList.push({ sym, reason: `新进缓冲区Top${signal.bufferN}(#${rank})`, price: signal.prices?.[sym] });
          }
        }
      }
    }
  ...
  if (cap > 0 && sellList.length > 0 && prevSignal) {
    const allPrevHoldings = { ...(prevSignal.holdings || {}), ...(prevSignal.bufferHoldings || {}) };
    sellTotal = sellList.reduce((s, x) => {
      const w = allPrevHoldings[x.sym] ?? 0;
      return s + cap * w;
    }, 0);
  }
        ...
        <div style={{marginBottom:14}}>
          ...
          {bufferList.length > 0 && (
            <div style={{marginBottom:10}}>
              ...
              <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
                {bufferList.map(({sym,reason,price})=>(
                  <div key={sym} style={{padding:'5px 10px',borderRadius:4,fontSize:10,
                    background:'#e8a02011',border:'1px solid #e8a02033'}}>
                    <span style={{fontWeight:700,color:'#e8a020',fontFamily:'monospace'}}>{sym}</span>
                    {price != null && <span style={{color:T.textMuted,marginLeft:4}}>${price.toFixed(1)}</span>}
                    <div style={{color:T.textMuted,fontSize:9,marginTop:1}}>{reason}</div>
                  </div>
                ))}
      ...
      {!signal.isDefensive && signal.scores.length > 0 && (
        <div style={{marginBottom:14}}>
          <div style={{fontSize:10,color:T.textMuted,marginBottom:6}}>
            当前排名（前{Math.min(signal.scores.length, 10)}），
            ✓ 为Top{signal.topN}持仓
          ...
          <div style={{display:'flex',gap:5,flexWrap:'wrap'}}>
            {signal.scores.slice(0, 10).map(({sym,score},i)=>{
              ...
              const tagColor = inCore ? borderColor : inBuffer ? '#e8a020' : (score>=0?'#4fc86e44':'#ee444433');
              ...
              return (
                <div key={sym} style={{
                  ...
                  #{i+1} {sym} {score>=0?'+':''}{(score*100).toFixed(1)}%{inCore&&<span style={{color:borderColor}}> ✓</span>}{inBuffer&&<span style={{color:'#e8a020'}}> ◉</span>}
                </div>
              );
            })}
          </div>
...
//  QQQ 成分股轮动 — 策略规则卡
// ══════════════════════════════════════════
function QqqStrategyRuleCard({ T, darkMode, stratParams }) {
  ...
  const filterLabel = marketFilter==='none'?'无过滤':marketFilter==='ma50'?'QQQ<MA50→现金':marketFilter==='ma100'?'QQQ<MA100→现金':'QQQ<MA200→现金';
  const bufferN = bufferEnabled ? Math.round(topN * 1.5) : topN;
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px' }}>
            <div>
              ...
              <div>· 对 QQQ 全部成分股（约90只）按指定动能指标计算得分并排名</div>
            ...
            <div>
              ...
              <div>· 不含交易成本和滑点</div>
              <div>· 回测使用当前 QQQ 成分股，存在幸存者偏差</div>
            </div>
          </div>
        </div>
      )}
...
export default function App() {
  ...
  const [wfoMode,     setWfoMode]     = useState("auto");   // 'fixed' | 'auto'
  const [appliedParams,setAppliedParams]= useState(null);   // 上次回测所用参数（WFO 固定参数模式用）
  ...
  const runStratBacktest = useCallback((overrideParams)=>{
    ...
    setAppliedParams({...params}); // 记录本次回测参数，供 WFO 固定参数模式使用
    setWfoMode('fixed');            // 运行回测后，WFO 自动切换到固定参数模式
    const qqqCloses=histData.get('__QQQ__').closes;
  ...
  const handleRunWFO = useCallback(async ()=>{
    ...
    const qqqCloses=histData.get('__QQQ__').closes;
    const stockData=new Map([...histData].filter(([k])=>k!=='__QQQ__'));
    const fixedArg = wfoMode === 'fixed' ? appliedParams : null; // 固定参数模式 or 自动寻优
    const result=runWFO(stockData,histTs,qqqCloses,wfoOptMetric,fixedArg);
    ...
    <div style={{minHeight:"100vh", background:T.pageBg, color:T.text, fontFamily:"'IBM Plex Mono',monospace"}}>
      ...
      {activeTab === "backtest" && (
        <div>
          ...
          {/* 策略规则卡 */}
          <QqqStrategyRuleCard T={T} darkMode={darkMode} stratParams={stratParams} />
                ...
                <div style={{padding:"16px 20px",background:T.cardBg,border:`1px solid ${T.border}`,borderTop:"none",borderRadius:"0 0 8px 8px"}}>
                  ...
                  <div style={{display:"flex",gap:0,marginBottom:14,border:`1px solid ${T.border}`,borderRadius:8,overflow:"hidden",width:"fit-content"}}>
                    {[
                      ...
                      {id:"auto",  icon:"🔍", label:"自动寻优 WFO",       desc:"IS Grid Search 选最优"},
                    ].map(({id,icon,label,desc})=>(
                      <button key={id} onClick={()=>setWfoMode(id)} style={{
                        ...
                        color: wfoMode===id?(darkMode?"#cc99ff":"#5522aa"):T.textSub,
                        fontWeight: wfoMode===id?600:400,
                      }}>
                        {icon} {label}
                  ...
                  {wfoMode==="fixed"&&(
                    <div style={{marginBottom:14,padding:"10px 14px",
                      background:darkMode?"#0a2a0a":"#f0fff0",
                      border:`1px solid ${darkMode?"#1a5a1a":"#88cc88"}`,borderRadius:7}}>
                      {appliedParams ? (
                        <>
                  ...
                  <button disabled={wfoRunning||(wfoMode==="fixed"&&!appliedParams)} onClick={handleRunWFO}
                    style={{padding:"6px 20px",borderRadius:6,fontFamily:"inherit",fontSize:12,
                      ...
                      border:"1px solid #9966ee",color:darkMode?"#cc99ff":"#ffffff",
                            ...
                            <thead>
                              <tr>
                                {[
                                  ...
                                  {h:"TopN",              note:isFixed?"固定参数":"IS→OOS 参数",group:"param"},
                                  {h:"动能回看",          note:isFixed?"固定参数":"IS→OOS 参数",group:"param"},
                                  {h:"调仓频率",          note:isFixed?"固定参数":"IS→OOS 参数",group:"param"},
                                  {h:"市场过滤",          note:isFixed?"固定参数":"IS→OOS 参数",group:"param"},
                                  ...(!isFixed?[{h:`IS ${optLabel}`,note:"in-sample 最优得分",group:""}]:[]),
                                  {h:"IS CAGR",      note:"IS 参考（仅展示）",group:"is"},
                                  {h:"IS Sharpe",    note:"IS 参考（仅展示）",group:"is"},
                              ...
                              {wfoResult.windowResults.map((w,i)=>{
                                ...
                                return (
                                  <tr key={i} style={{background:i%2===0?T.cardBg:T.cardBg2}}>
                                    ...
                                    {/* IS 参考绩效（两模式均显示，固定模式标注"参考"）*/}
                                    <td style={{padding:"7px 10px",fontFamily:"monospace",color:"#888888"}}>
                                      {w.inSampleCAGR!=null?fmtPct(w.inSampleCAGR,1):"—"}
                                    </td>
                                    <td style={{padding:"7px 10px",fontFamily:"monospace",color:"#888888"}}>
                                      {w.inSampleSharpe!=null?w.inSampleSharpe.toFixed(2):"—"}
                                    </td>
                                    {/* OOS 实际绩效 */}
