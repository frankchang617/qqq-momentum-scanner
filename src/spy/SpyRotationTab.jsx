/**
 * SpyRotationTab.jsx — SPY 成分股轮转策略
 *
 * Props:
 *   T         : 主题色对象
 *   darkMode  : boolean
...
import { backtestSpyRotation, buildSpyBenchmark, paramLabelSpy } from './strategies/spyRotation.js';
import { runSpyGridSearch } from './optimization/spyGridSearch.js';
import { runSpyWFO } from './optimization/spyWfo.js';
...
function EquityCurveChart({ stratEq, spyEq, timestamps, T }) {
  ...
  const qp = (spyEq||[]).map((v,i) => `${tx(i)},${ty(v)}`).join(' ');
  ...
  return (
    <svg width={W} height={H} style={{ display:'block' }}>
      ...
      {spyEq && spyEq.length > 0 && <polyline points={qp} fill="none" stroke={T.textMuted} strokeWidth="1.5" strokeDasharray="5,3" opacity="0.7"/>}
...
// ─── 双值对比指标卡 ────────────────────────────────────────────────────────────
function DualMetricCard({ label, strat, spy, unit='', higherBetter=true, fmtFn=v=>v?.toFixed(2), alwaysRed=false, T }) {
  const good = alwaysRed ? false : higherBetter ? (strat??0) >= (spy??strat??0) : (strat??0) <= (spy??strat??0);
  return (
    <div style={{ padding:'10px 14px', background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:8, minWidth:130 }}>
      ...
      {spy != null && (
        <div style={{ fontSize:10, color:T.textMuted, marginTop:2 }}>SPY {fmtFn(spy)}{unit}</div>
      )}
    </div>
  );
}

// ─── calcMetrics（小数）→ 百分比格式转换 ──────────────────────────────────────
        ...
        <div style={{
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 24px' }}>
            ...
            <div>
              ...
              <div>· 市场过滤：SPY 收盘 &lt; SMA200 → 全仓切换至 <b style={{ color: '#e8883a' }}>{params.defensiveAsset}</b></div>
              <div>· 防御资产可选：CASH（现金）/ SPY（买入持有）/ SHY（短期国债）</div>
            </div>
            <div>
...
// ─── 主组件 ──────────────────────────────────────────────────────────────────
export default function SpyRotationTab({ T, darkMode }) {
  ...
  const [myHoldings, setMyHoldings] = useState(() => {
    try { return JSON.parse(localStorage.getItem('spy_rotation_holdings') || 'null'); }
    catch { return null; }
  });
  useEffect(() => {
    if (myHoldings !== null) localStorage.setItem('spy_rotation_holdings', JSON.stringify(myHoldings));
    else localStorage.removeItem('spy_rotation_holdings');
  }, [myHoldings]);
  ...
  const loadHistData = useCallback(async () => {
    ...
    try {
      ...
      for (const [sym, rows] of rawMap) {
        const closesArr = new Array(Ns).fill(null);
        const opensArr  = new Array(Ns).fill(null);
      ...
      try {
        ...
        if (shyRaw) {
          const shyClosesArr = new Array(Ns).fill(null);
          const shyOpensArr  = new Array(Ns).fill(null);
      ...
      setHistDataLocal(aligned); setHistTsLocal(spyTs); setHistLoading(false);
  ...
  const handleRunBacktest = useCallback(() => {
    ...
    setTimeout(() => {
      try {
        const bt = backtestSpyRotation(histDataLocal, histTsLocal, params);
        ...
        const spyCloses   = histDataLocal.get('__SPY__')?.closes ?? histDataLocal.get('__SPY__');
        const startOffset = histTsLocal.indexOf(bt.timestamps[0]);
        const spyEq       = buildSpyBenchmark(spyCloses, startOffset, startOffset + bt.timestamps.length);
        const spyMetrics  = calcMetrics(spyEq, bt.timestamps);
        setBtResult({ ...bt, metrics, portMetrics: toPortMetrics(metrics), spyEq, spyMetrics, spyPortMetrics: toPortMetrics(spyMetrics) });
      } catch (e) { console.error('SPY rotation backtest error:', e); }
      setBtRunning(false);
    }, 50);
  }, [histDataLocal, histTsLocal, params]);

  // ── 应用参数并立即运行回测 ──
  const handleApplyAndBacktest = useCallback((rowParams) => {
    setParams({ ...rowParams });
    setAppliedParams({ ...rowParams });
    setWfoMode('fixed');
    setBtRunning(true);
    setBtResult(null);
    setTimeout(() => {
      try {
        const bt = backtestSpyRotation(histDataLocal, histTsLocal, rowParams);
        if (!bt) { setBtRunning(false); return; }
        const metrics = calcMetrics(bt.equityCurve, bt.timestamps);
        if (!metrics) { setBtRunning(false); return; }
        const spyCloses   = histDataLocal.get('__SPY__')?.closes ?? histDataLocal.get('__SPY__');
  ...
  const handleRunGridSearch = useCallback(async () => {
    ...
    try {
      const res = await runSpyGridSearch(
        histDataLocal, histTsLocal, 0, null,
        (done, total) => setGsProgress({ done, total })
      );
      setGsResult(res);
    } catch (e) { console.error('SPY grid search error:', e); }
    setGsRunning(false);
  }, [histDataLocal, histTsLocal]);
  ...
  const handleRunWFO = useCallback(async () => {
    ...
    try {
      const res = await runSpyWFO(
        histDataLocal, histTsLocal, wfoOptMetric,
        (phase, done, total) => {
      ...
      setWfoResult(res);
    } catch (e) { console.error('SPY WFO error:', e); }
    setWfoRunning(false);
    setWfoPhase('');
  }, [histDataLocal, histTsLocal, wfoOptMetric, wfoMode, appliedParams]);

  // ── 当前操作信号（基于手动回测参数）──
  const signal = useMemo(() => {
    if (!histDataLocal || !histTsLocal) return null;
    const N      = histTsLocal.length;
    const sigIdx = N - 1;
    const date   = new Date(histTsLocal[sigIdx] * 1000).toISOString().slice(0, 10);

    const spyData   = histDataLocal.get('__SPY__');
    const spyCloses = spyData?.closes ?? spyData;
    const shyData   = histDataLocal.get('SHY');
    const shyCloses = shyData?.closes ?? shyData ?? null;
    const symbols   = [...histDataLocal.keys()].filter(k => k !== '__SPY__' && k !== 'SHY');

    const symCloses = new Map();
    for (const sym of symbols) {
      const d = histDataLocal.get(sym);
      symCloses.set(sym, d?.closes ?? d);
    }

    let isDefensive = false, sma200 = null, spyNow = spyCloses?.[sigIdx];
    if (params.marketFilter && sigIdx >= 200) {
      let sum = 0, cnt = 0;
      for (let k = sigIdx - 199; k <= sigIdx; k++) {
        if (spyCloses[k] != null) { sum += spyCloses[k]; cnt++; }
      }
      sma200 = cnt === 200 ? sum / 200 : null;
      if (sma200 !== null && spyNow < sma200) isDefensive = true;
    }

    if (isDefensive) {
      return { date, isDefensive: true, holdings: {}, prices: {}, spyNow, sma200,
    ...
    return {
      date, isDefensive: false, spyNow, sma200,
  ...
  if (signal && prevSignal && !isFirstSignal) {
    ...
    if (!wasDefensive && !isDefensive) {
      ...
      holdList.push({ sym: signal.defensiveAsset || 'CASH', reason: '继续防御', price: null });
    }
    if (isDefensive && !wasDefensive) {
      buyList.push({ sym: signal.defensiveAsset || 'CASH', reason: '市场过滤触发 → 切换防御资产', price: null });
    }
  }
  ...
  if (cap > 0 && sellList.length > 0)
    sellTotal = sellList.reduce((s, x) => s + cap * (prevSignal?.holdings?.[x.sym] ?? 0), 0);
  ...
  // ── 数据未加载 ──
  if (!histDataLocal || !histTsLocal || histTsLocal.length === 0) {
      ...
      <div style={{ padding: '20px 28px' }}>
        ...
        <div style={{ padding: '16px 20px', background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: histLoading ? 12 : 0 }}>
            ...
            {histDataLocal && !histLoading && (
              <span style={{ fontSize: 11, color: '#00aa44' }}>
                ✓ 已加载 {histDataLocal.size - 1} 只股票 × {histRange === '2y' ? '2年' : histRange === '3y' ? '3年' : histRange === '5y' ? '5年' : '10年'}数据
              </span>
            )}
          </div>
          {histLoading && (
        ...
        {!histLoading && (
          <div style={{ padding: '48px 28px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 11, color: T.textSub }}>
              选择历史深度后点击「加载历史数据」，约 {SPY_COMPONENTS.length} 只 S&P 500 成分股 + SPY ETF
            </div>
          </div>
        )}
      </div>
    ...
    <div style={{ padding: '20px 28px' }}>
      ...
      <div style={{ padding: '16px 20px', background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
          ...
          <ParamRow label="市场过滤" T={T} value={params.marketFilter} onChange={v => setParams(p => ({ ...p, marketFilter: v }))}
            options={[{ value: false, label: '关闭' }, { value: true, label: 'SPY < SMA200' }]} />
          {params.marketFilter && (
            <ParamRow label="防御资产" T={T} value={params.defensiveAsset} onChange={v => setParams(p => ({ ...p, defensiveAsset: v }))}
              options={[{ value: 'CASH', label: '现金' }, { value: 'SPY', label: 'SPY' }, { value: 'SHY', label: 'SHY' }]} />
          )}
        </div>
        ...
        {btResult && btResult.portMetrics && (
          <div style={{ marginTop: 20 }}>
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 12 }}>
              <DualMetricCard label="CAGR（年化收益）" strat={btResult.portMetrics.cagr} spy={btResult.spyPortMetrics?.cagr} unit="%" fmtFn={v=>v.toFixed(1)} T={T}/>
              <DualMetricCard label="Sharpe Ratio" strat={btResult.portMetrics.sharpe} spy={btResult.spyPortMetrics?.sharpe} fmtFn={v=>v.toFixed(2)} T={T}/>
              <DualMetricCard label="最大回撤 MDD" strat={btResult.portMetrics.mdd} spy={btResult.spyPortMetrics?.mdd} unit="%" higherBetter={false} alwaysRed fmtFn={v=>v.toFixed(1)} T={T}/>
              <DualMetricCard label="累积收益" strat={btResult.portMetrics.total} spy={btResult.spyPortMetrics?.total} unit="%" fmtFn={v=>v.toFixed(1)} T={T}/>
            ...
            {(() => {
              ...
              const sEq = btResult.spyEq;
              const finalStrat = !isNaN(c) && eq?.length  ? c * eq[eq.length-1]   : null;
              const finalSPY   = !isNaN(c) && sEq?.length ? c * sEq[sEq.length-1] : null;
                ...
                <div style={{ display:'flex', alignItems:'center', gap:14, flexWrap:'wrap', marginBottom:16, padding:'10px 16px', background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:8 }}>
                  ...
                  {finalStrat != null && (
                    <div style={{ display:'flex', gap:20, flexWrap:'wrap', alignItems:'center' }}>
                      ...
                      <span style={{ fontSize:12, color:T.textMuted }}>SPY最终:&nbsp;
                        <span style={{ fontFamily:'monospace', fontWeight:700, fontSize:14, color:finalSPY!=null&&finalSPY>=c?'#00aa44':'#ee3344' }}>{finalSPY!=null?`$${fmtM(finalSPY)}`:'—'}</span>
                      </span>
                      {finalSPY != null && <span style={{ fontSize:11, fontFamily:'monospace', color:finalStrat>=finalSPY?'#00aa44':'#ee3344' }}>
                        {finalStrat>=finalSPY?'▲':'▼'} {((finalStrat-finalSPY)/finalSPY*100).toFixed(1)}% vs SPY
                      </span>}
                    </div>
                  )}
                </div>
            ...
            <div style={{ background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:8, padding:'14px 16px', marginBottom:12, overflowX:'auto' }}>
              ...
              <EquityCurveChart stratEq={btResult.equityCurve} spyEq={btResult.spyEq} timestamps={btResult.timestamps} T={T}/>
            </div>
            ...
            <div style={{ background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:8, padding:'14px 16px', marginBottom:12, overflowX:'auto' }}>
              ...
              <AnnualBarsChart stratAnnual={btResult.portMetrics.annualRets} spyAnnual={btResult.spyPortMetrics?.annualRets ?? {}} T={T}/>
            </div>
      ...
      {signal && (
        <div style={{
          ...
          {params.marketFilter && signal.spyNow != null && signal.sma200 != null && (
            <div style={{ fontSize: 11, color: T.textSub, marginBottom: 10 }}>
              SPY <strong style={{ color: T.textBright }}>${signal.spyNow.toFixed(1)}</strong>
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {sellList.map(({ sym, reason, price }) => (
                      <div key={sym} style={{ padding: '5px 10px', borderRadius: 4, fontSize: 10, background: '#ee334411', border: '1px solid #ee334433' }}>
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {buyList.map(({ sym, reason, price }) => (
                      <div key={sym} style={{ padding: '5px 10px', borderRadius: 4, fontSize: 10, background: '#4fc86e11', border: '1px solid #4fc86e33' }}>
                ...
                <div style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                    {holdList.map(({ sym, reason, price }) => (
                      <div key={sym} style={{ padding: '5px 10px', borderRadius: 4, fontSize: 10, background: '#4488ee11', border: '1px solid #4488ee33' }}>
          ...
          {!isFirstSignal && cap > 0 && (sellList.length > 0 || buyList.length > 0) && (
            <div style={{ padding: '8px 12px', marginBottom: 12, borderRadius: 6, background: T.pageBg, border: `1px solid ${T.border}`, fontSize: 11, color: T.textSub, lineHeight: 1.8 }}>
              <div style={{ fontWeight: 700, marginBottom: 2, color: T.textBright }}>💰 资金流转摘要</div>
              {sellList.length > 0 && <div>🔴 卖出回收：<b style={{ color: '#ee3344', fontFamily: 'monospace' }}>{fmtMoney(sellTotal)}</b></div>}
              {buyList.length > 0  && <div>🟢 买入需要：<b style={{ color: '#4fc86e', fontFamily: 'monospace' }}>{fmtMoney(buyTotal)}</b></div>}
              <div>
                {netFlow > 0.005 ? (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            ...
            <input
              ...
              style={{ width: 100, padding: '4px 8px', borderRadius: 4, background: T.pageBg, border: `1px solid ${T.border}`, color: T.textBright, fontSize: 12, fontFamily: 'inherit' }}
            />
          </div>
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {Object.entries(signal.holdings).map(([sym, w]) => {
                  ...
                  return (
                    <div key={sym} style={{ padding: '8px 12px', borderRadius: 6, minWidth: 110, background: T.pageBg, border: `1px solid ${T.border}` }}>
      ...
      <div style={{ padding: '16px 20px', background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 8, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', marginBottom: gsRunning ? 12 : 0 }}>
          <button onClick={handleRunGridSearch} disabled={gsRunning} style={{
            ...
            color: gsRunning ? T.textMuted : '#fff', opacity: gsRunning ? 0.7 : 1,
          }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            <span style={{ fontSize: 11, color: T.textSub }}>$</span>
            <input type="number" value={investAmount} min={0} step={1000} onChange={e => setInvestAmount(e.target.value)}
              style={{ width: 100, padding: '4px 8px', borderRadius: 4, background: T.pageBg, border: `1px solid ${T.border}`, color: T.textBright, fontSize: 12, fontFamily: 'inherit' }}/>
          </div>

          {gsResult && !gsRunning && (
            <span style={{ fontSize: 11, color: '#00aa44' }}>✓ 完成 {gsResult.length} 种有效组合</span>
          )}
        </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'separate', borderSpacing: 0, fontSize: 11 }}>
                ...
                <tbody>
                  {gsResult.slice(0, 30).map((r, i) => {
                    ...
                    const isTop    = i === 0;
                    return (
                      <tr key={i} style={{ background: isTop ? (darkMode ? '#001a00' : '#f0fff0') : 'transparent', cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '5px 8px', borderBottom: `1px solid ${T.border}44` }}>
                          <button onClick={e => { e.stopPropagation(); handleApplyAndBacktest(r.params); }} style={{
                            padding: '3px 10px', borderRadius: 4, cursor: 'pointer',
                            fontFamily: 'inherit', fontSize: 10, fontWeight: 600,
                            background: darkMode ? '#004488' : '#0055cc',
      ...
      <div style={{ marginBottom: 20 }}>
        <button onClick={() => setShowWfo(v => !v)} style={{
          ...
          <div style={{ padding: '16px 20px', background: T.cardBg, border: `1px solid ${T.border}`, borderTop: 'none', borderRadius: '0 0 8px 8px' }}>
              ...
              {[
                ...
                return (
                  <button key={m.val} onClick={() => !wfoRunning && setWfoMode(m.val)} style={{
                    ...
                    cursor: wfoRunning ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                    background: active ? '#9944ee22' : T.pageBg, border: 'none',
                    borderLeft: idx > 0 ? `1px solid ${T.border}` : 'none',
                    borderBottom: active ? '2px solid #9944ee' : '2px solid transparent',
                  }}>
                    <div style={{ fontSize: 11, fontWeight: 700, color: active ? '#cc99ff' : T.textSub, marginBottom: 2 }}>{m.icon} {m.title}</div>
            ...
            {wfoMode === 'fixed' && (
              <div style={{
                ...
                {appliedParams ? (
                  <>
                    <div style={{ color: '#4fc86e', fontWeight: 700, marginBottom: 6 }}>✅ 已选参数（将直接用于 OOS，不重新搜索）</div>
                    <div style={{ fontFamily: 'monospace', fontSize: 11, color: '#cc99ff', fontWeight: 600, padding: '5px 10px', background: '#cc99ff11', borderRadius: 5, border: '1px solid #cc99ff33', display: 'inline-block' }}>
                      {paramLabelSpy(appliedParams)}
                    </div>
                  ...
                  <div style={{ color: '#ee4444' }}>⚠ 尚未选择参数。请在「一键优化」结果表中点击「应用并回测」，再来运行固定参数验证。</div>
                )}
              </div>
            )}
            ...
            <button disabled={wfoRunning || (wfoMode === 'fixed' && !appliedParams)} onClick={handleRunWFO} style={{
            ...
            {wfoResult && (() => {
              ...
              const qm     = wfoResult.spyCombinedMetrics;
              ...
              return (
                <>
                  <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', marginBottom: 14, padding: '10px 14px', background: T.pageBg, border: `1px solid ${T.border}`, borderRadius: 7, fontSize: 10 }}>
                    <span style={{ color: T.textSub }}>模式：<b style={{ color: '#cc99ff' }}>{isFixed ? '固定参数 OOS 验证' : '自动寻优 WFO'}</b></span>
                    ...
                    {isFixed
                      ? <span style={{ color: T.textSub }}>参数：<b style={{ color: '#cc99ff' }}>{paramLabelSpy(wfoResult.fixedParams)}</b></span>
                  ...
                  <div style={{ padding: '8px 12px', marginBottom: 10, background: isFixed ? '#4fc86e14' : '#4488ee14', border: `1px solid ${isFixed ? '#4fc86e40' : '#4488ee40'}`, borderRadius: 6, fontSize: 10, color: T.textSub, lineHeight: 1.6 }}>
                    {isFixed
                      ? <>📌 <b style={{ color: '#4fc86e' }}>固定参数验证</b>：OOS 期间使用「<span style={{ color: '#cc99ff', fontWeight: 600 }}>{paramLabelSpy(wfoResult.fixedParams)}</span>」，<b style={{ color: T.textBright }}>完全固定，IS 期仅供参考</b>。</>
                      : <>⚡ <b style={{ color: '#88bbff' }}>自动寻优</b>：OOS 严格使用 IS 期选出的最优参数，<b style={{ color: T.textBright }}>不重新优化、不事后调参</b>。</>
                    }
                  </div>
                        ...
                        {wfoResult.windowResults.map((w, i) => {
                          ...
                          return (
                            <tr key={i} style={{ background: i % 2 === 0 ? T.cardBg : T.pageBg }}>
                              ...
                              <td style={{ padding: '7px 10px', fontFamily: 'monospace', color: isFixed ? ((w.inSampleCAGR ?? 0) >= 0 ? '#00aa44' : '#ee3344') : T.textSub }}>
                                {isScoreDisplay}
                              </td>
                              ...
                              <td style={{ padding: '7px 10px', fontFamily: 'monospace', color: (w.spyOutMetrics?.cagr ?? 0) >= 0 ? '#00aa44' : '#ee3344' }}>
                                {w.spyOutMetrics ? fmtPct(w.spyOutMetrics.cagr) : '—'}
                              </td>
                              {!isFixed && (
                                <td style={{ padding: '5px 8px' }}>
                                  <button onClick={() => handleApplyAndBacktest(bp)} style={{
                                    padding: '3px 10px', borderRadius: 4, cursor: 'pointer',
                                    fontFamily: 'inherit', fontSize: 10, fontWeight: 600,
                                    background: darkMode ? '#004488' : '#0055cc',
                                    border: '1px solid #4488ee', color: '#fff', whiteSpace: 'nowrap',
                  ...
                  {wfoResult.allOutEquity.length > 10 && (
                    <div style={{ background: T.pageBg, border: `1px solid ${T.border}`, borderRadius: 8, padding: '14px 16px', overflowX: 'auto', marginBottom: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 10, color: T.textSub }}>OOS 净值曲线（策略 蓝线 vs SPY 灰虚线）</span>
                        ...
                        {isFixed && <span style={{ fontSize: 9, color: '#4fc86e' }}>固定参数：{paramLabelSpy(wfoResult.fixedParams)}</span>}
                      </div>
                      <EquityCurveChart
                        stratEq={wfoResult.allOutEquity}
                        spyEq={wfoResult.spyWfoEq ?? []}
                  ...
                  <WfoSummaryTable
                    cm={cm}
                    benchmarks={[{ label: 'SPY 基准', metrics: qm }]}
                  ...
                  {btResult?.metrics && cm && (() => {
                    ...
                    const rows = [
                      ...
                      { mode: 'SPY Buy & Hold（同 OOS 期间基准）', cagr: qm?.cagr, sharpe: qm?.sharpe, mdd: qm?.mdd, total: qm?.totalReturn },
                    ];
                    return (
